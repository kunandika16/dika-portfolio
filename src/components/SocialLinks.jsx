    ...
    const fetchProfile = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .select(
            "linkedin_connect, instagram_connect, youtube_connect, github_connect, tiktok_connect"
          )
          .eq("id", 1)
          .single();
        ...
        if (data) {
          setSocialLinks((prev) =>
            prev.map((link) => {
              switch (link.name) {
                case "LinkedIn":
                  return data.linkedin_connect
                    ? { ...link, url: data.linkedin_connect }
                    : link;
                case "Instagram":
                  return data.instagram_connect
                    ? { ...link, url: data.instagram_connect }
                    : link;
                case "YouTube":
                  return data.youtube_connect
                    ? { ...link, url: data.youtube_connect }
                    : link;
                case "GitHub":
                  return data.github_connect
                    ? { ...link, url: data.github_connect }
                    : link;
                case "TikTok":
                  return data.tiktok_connect
                    ? { ...link, url: data.tiktok_connect }
                    : link;
                default:
                  return link;
              }
